...
export default function Home() {
  ...
  async function fetchFeed(accessToken) {
    ...
    try {
      ...
      const uri = `http://localhost:5000/feed/${address}`
      ...
      const items = result.feed.map(({score, str, pubId, suggestedBy}) => ({score, str, id: pubId, suggestedBy}))
      setPersonalizedFeed(items)
      setRelatedTags(result.userTags)  // [(score, tagId, tag)]
  ...
  return (
    <div>
      <div>
        <p className="text-3xl text-slate-400 mb-4">Smart Cookies</p>
        ...
        view === 'personalized' && authenticated && (
          <div>
            ...
            {personalizedFeed.length > 0 && (
              <div className='mt-4'>
                Suggested based on your tags: 
                <p>
                  {
                    relatedTags.map(([score, _, tag]) => (
                      <span className='inline-block rounded-lg bg-zinc-300 px-3 m-1' key={tag}>
                        {tag}: {score.toString()}
                      </span>
                    ))
                  }
